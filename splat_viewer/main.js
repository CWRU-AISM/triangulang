...
// Try to parse initial viewMatrix from URL hash or use default
let defaultViewMatrix;
try {
    defaultViewMatrix = JSON.parse(decodeURIComponent(location.hash.slice(1)));
} catch(e) {
    defaultViewMatrix = [
        1, 0, 0, 0,
        0, 1, 0, 0,
...
// Scene bounds for orbit controls
window._sceneCenter = [0, 0, 0];
window._sceneExtent = 5;
window._orbitDist = 5;
window._minDist = 0.5;
window._maxDist = 30;

function clampCamera(vm) {
    // Extract camera position from view matrix (column-major)
    let inv = invert4(vm);
    let cx = inv[12], cy = inv[13], cz = inv[14];
    let sc = window._sceneCenter;
    let dx = cx - sc[0], dy = cy - sc[1], dz = cz - sc[2];
    let dist = Math.sqrt(dx*dx + dy*dy + dz*dz);

    // Clamp distance
    if (dist < window._minDist || dist > window._maxDist) {
        let targetDist = Math.max(window._minDist, Math.min(window._maxDist, dist));
        let scale = targetDist / Math.max(dist, 0.001);
        inv[12] = sc[0] + dx * scale;
        inv[13] = sc[1] + dy * scale;
        inv[14] = sc[2] + dz * scale;
        return invert4(inv);
    }
    return vm;
}
...
async function main() {
    ...
    worker.onmessage = (e) => {
        if (e.data.buffer) {
            ...
            if (!window._cameraFitted && vertexCount > 1000 && splatData.length > 0) {
                ...
                let cx = 0, cy = 0, cz = 0;
                const dv = new DataView(splatData.buffer);
                const step = Math.max(1, Math.floor(N / 5000));
                let count = 0;
                for (let i = 0; i < N; i += step) {
                    cx += dv.getFloat32(i * rowLen, true);
                    cy += dv.getFloat32(i * rowLen + 4, true);
                    cz += dv.getFloat32(i * rowLen + 8, true);
                    count++;
                }
    ...
    window.addEventListener(
        ...
        (e) => {
            ...
            let d = window._orbitDist || 4;
            if (e.shiftKey) {
                // Pan
                let panScale = d * 0.3;
                inv = translate4(inv,
                    (e.deltaX * scale * panScale) / innerWidth,
                    (e.deltaY * scale * panScale) / innerHeight, 0);
            } else {
                // Scroll = zoom (move along view axis)
                let zoomAmt = (e.deltaY * scale * d * 0.3) / innerHeight;
                inv = translate4(inv, 0, 0, -zoomAmt);
            }
            viewMatrix = invert4(inv);
            viewMatrix = clampCamera(viewMatrix);
        },
        { passive: false },
    );
    ...
    canvas.addEventListener("mousemove", (e) => {
        ...
        if (down == 1) {
            ...
            let panScale = d * 0.5;
            inv = translate4(inv,
                (-panScale * (e.clientX - startX)) / innerWidth,
                0,
                (panScale * (e.clientY - startY)) / innerHeight);
            viewMatrix = invert4(inv);
            viewMatrix = clampCamera(viewMatrix);
    ...
    const frame = (now) => {
        ...
        if (activeKeys.includes("ArrowUp")) {
            if (shiftKey) {
                ...
                inv = translate4(inv, 0, 0, moveSpeed);
            }
        }
        ...
        if (activeKeys.includes("ArrowLeft"))
            inv = translate4(inv, -moveSpeed, 0, 0);
        if (activeKeys.includes("ArrowRight"))
            inv = translate4(inv, moveSpeed, 0, 0);
        if (activeKeys.includes("KeyA")) inv = rotate4(inv, -rotSpeed, 0, 1, 0);
        if (activeKeys.includes("KeyD")) inv = rotate4(inv, rotSpeed, 0, 1, 0);
        if (activeKeys.includes("KeyQ")) inv = rotate4(inv, rotSpeed, 0, 0, 1);
        if (activeKeys.includes("KeyE")) inv = rotate4(inv, -rotSpeed, 0, 0, 1);
        if (activeKeys.includes("KeyW")) inv = rotate4(inv, rotSpeed * 0.5, 1, 0, 0);
        if (activeKeys.includes("KeyS")) inv = rotate4(inv, -rotSpeed * 0.5, 1, 0, 0);

        // Clamp camera to scene bounds
        viewMatrix = clampCamera(invert4(inv));
...
// Mask overlay via postMessage from parent frame
window.addEventListener('message', (e) => {
    if (!e.data || !e.data.type) return;

    if (e.data.type === 'applyMask') {
        const mask = e.data.mask; // Uint8Array, one byte per Gaussian (0-255 relevancy)
        ...
        if (mask.length !== N) {
            console.warn(`Mask length ${mask.length} != Gaussian count ${N}`);
            return;
        }
        ...
        function turbo(t) {
            ...
            const idx = t * (TURBO.length - 1);
            const lo = Math.floor(idx), hi = Math.min(lo + 1, TURBO.length - 1);
            const f = idx - lo;
            return [
                TURBO[lo][0] * (1-f) + TURBO[hi][0] * f,
                TURBO[lo][1] * (1-f) + TURBO[hi][1] * f,
                TURBO[lo][2] * (1-f) + TURBO[hi][2] * f,
            ];
        }
        ...
        // Send modified buffer to worker and trigger re-sort
        window._workerRef.postMessage({
            buffer: splatData.buffer,
            vertexCount: N,
        });
        // Trigger re-sort via the global forceResort function
        setTimeout(() => { if (window._forceResort) window._forceResort(); }, 200);
        console.log('Applied ' + mode + ' overlay');
    }
